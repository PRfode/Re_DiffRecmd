...
class LDiffRec_b(BaseModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--emb_path', type=str, default='../embeddings/', help='Path for item embeddings')
    ...
    def __init__(self, args, corpus):
        ...
        self.sampling_steps = args.sampling_steps if (args.sampling_steps > 0 and args.sampling_steps < self.steps) else self.steps
        ...
        # 1. 加载物品嵌入
        emb_path = f'{self.emb_path}{args.dataset}/item_emb.npy'
        try:
            item_emb = torch.from_numpy(np.load(emb_path, allow_pickle=True))
            assert len(item_emb) == corpus.n_items
        except:
            # 如果没有预训练嵌入，使用随机初始化
            print(f"Warning: Item embedding not found at {emb_path}, using random initialization")
            item_emb = torch.randn(corpus.n_items, self.in_dims[0] if len(self.in_dims) > 0 else 100)
        ...
        # 3. 扩散模型的MLP（在隐空间操作）
        # 获取隐空间维度
        latent_size = self.in_dims[-1] if len(self.in_dims) > 0 else 100
        
        # 构建MLP维度：确保输入维度与隐空间维度匹配
        if isinstance(self.mlp_dims, list) and len(self.mlp_dims) > 0:
            mlp_out_dims = self.mlp_dims + [latent_size]
            mlp_in_dims = mlp_out_dims[::-1]
        else:
            # 默认MLP结构
            mlp_out_dims = [latent_size * 2, latent_size]
            mlp_in_dims = mlp_out_dims[::-1]
    ...
    def forward(self, feed_dict):
        ...
        elbo = torch.mean(diffusion_losses / pt)
        ...
        if self.reweight: # reweight = True
            total_loss = lamda * elbo + vae_loss
        else:
            total_loss = elbo + lamda * vae_loss
        ...
        # self.update_count += 1 //在LGDRunner更新
        self.update_count_vae += 1
        
        # 10. 记录损失用于调试
        self.train_losses.append(total_loss.item())
        self.ae_losses.append(vae_loss.item())
        self.diff_losses.append(elbo.item())
        ...
        return {'loss': total_loss, 'prediction': torch.zeros(batch_size, 1, device=self.device)}

    def loss(self, out_dict):
        return out_dict['loss']

    # ========================================================================== #
    # gaussian_diffusion.py 相关函数
    # ========================================================================== #
    ...
    # ========================================================================== #
    # gaussian_diffusion.py 相关函数 (end)
    # ========================================================================== #

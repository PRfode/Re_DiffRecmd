...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
        ...
        super(GaussianDiffusion, self).__init__()  # 移到前面
        ...
        # 使用 register_buffer 注册缓冲区，确保设备同步
        # self.register_buffer('Lt_history', th.zeros(steps, history_num_per_term, dtype=th.float64))
        # self.register_buffer('Lt_count', th.zeros(steps, dtype=th.int64))
    ...
    #     alphas_cumprod_prev = th.cat([th.tensor([1.0]).to(self.device), alphas_cumprod[:-1]])
    #     alphas_cumprod_next = th.cat([alphas_cumprod[1:], th.tensor([0.0]).to(self.device)])
    ...
    #     self.register_buffer('log_one_minus_alphas_cumprod', th.log(1.0 - alphas_cumprod))
    ...
    #         th.cat([posterior_variance[1].unsqueeze(0), posterior_variance[1:]])
    #     )

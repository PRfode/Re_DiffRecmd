...
class DiffRec_c(BaseModel):
    ...
    def _build_diffusion_params(self):
        # 1. Betas 计算 (Strictly following original)
        ...
        # 【修正】引入 alphas_cumprod_prev，用于正确计算 t-1 时刻的状态
        ...
        # 【修正】使用 Log Variance 并进行截断 (Clipped)，这在原论文 inference 中被使用
        # 避免方差过小导致数值问题
    ...
    def forward(self, feed_dict):
        ...
        if self.reweight:
            # 【修正】使用 alphas_cumprod_prev 计算 SNR_prev，避免索引 -1 的问题
    ...
    def inference(self, feed_dict):
        ...
        with torch.no_grad():
            for i in indices:
                ...
                # Inference Clamping (Recommended for stability)
                ...
                if i > 0:
                    ...
                    # 【修正】使用 log_variance_clipped，数值更稳定
